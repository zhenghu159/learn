---
title: "Introduction to single-cell reference mapping"
output: 
  html_notebook: 
    toc: yes
    number_sections: yes
...
然后我们将这些锚点传递给 IntegrateData() 函数，该函数返回一个 Seurat 对象。  
返回的对象将包含一个新的 Assay，其中包含所有细胞的集成（或“批量校正”）表达矩阵，使它们能够被联合分析。
```{r}
...
Seurat 还支持将 reference 数据（或 meta data）投射到 query 对象上。虽然许多方法是保守的（两个过程都从识别anchors开始），但数据transfer和integration之间有两个重要区别：  
1.In data transfer, Seurat 不会更正或修改query表达数据。  
2.In data transfer, Seurat 有一个选项（默认设置）将 reference 的 PCA 结构投射到 query 上，而不是使用 CCA 学习联合结构。我们通常建议在 scRNA-seq 数据集之间投射数据时使用此选项。  
找到 anchors 后，我们使用 `TransferData()`  函数根据 reference 数据（参考细胞类型标签的向量）对 query cells 进行分类。`TransferData()` 返回一个包含预测 ID 和预测分数的矩阵，我们可以将其添加到 query metadata 中。
```{r}
...
在 Seurat v4 中，我们还启用了将query投影到reference UMAP 结构上。这可以通过计算reference UMAP 模型然后调用 `MapQuery()` 而不是 `TransferData()` 来实现。
```{r}
...
我们现在可以在reference旁边可视化query cells。
